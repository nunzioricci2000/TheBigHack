...
//                UIAlertController.show(error: STError.generic(text: "Enabled: \(Permission.exploreCatalog.appModesEnabled) - \(Permission.exploreCatalog.userTypesEnabled)"),
//                                       from: self)
//                return
//            }

            self?.navigationController?.pushViewController(BoardListPresenter(param: Void()).start(), animated: true)
