...
class WriteDataManager {
    class WriteData {
        ...
        init(data: Data, charactheristic: CBCharacteristic, completion: @escaping (Bool) -> Void) {
            self.data = data
    ...
    private func dequeueCommand() {
        ...
        sendWrite(command.data, characteristic: command.charactheristic) { [weak self] success in
    ...
    private func sendWrite(_ data: Data, characteristic: CBCharacteristic, completion: @escaping (Bool) -> Void) {
        if debug {
