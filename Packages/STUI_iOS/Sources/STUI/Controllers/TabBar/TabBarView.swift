...
public class TabBarView: UIView {
    ...
    public override func layoutSubviews() {
        ...
        path.addLine(to: CGPoint(x: stackView.frame.width / 2.0 - 30.0, y: 25.0))
        path.addArc(withCenter: CGPoint(x: stackView.frame.width / 2.0,
                                        y: 25.0),
                    radius: 30.0,
                    startAngle: CGFloat(Double.pi),
                    endAngle: 0.0,
                    clockwise: false)
        path.addLine(to: CGPoint(x: stackView.frame.width, y: 25.0))
        path.addLine(to: CGPoint(x: stackView.frame.width, y: frame.height + 100.0))
